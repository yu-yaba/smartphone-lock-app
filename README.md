...
- **サービス構成**: `OverlayLockService` がカウントダウン付きオーバーレイを描画し全画面タッチを食い止める。`LockMonitorService`（Foreground + UsageStats監視）が設定アプリ等を検知して即座に UI を取り戻す。デバッグビルドではオーバーレイ上に即時解除ボタンを表示。
