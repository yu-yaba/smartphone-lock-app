...
## 1. 概要
本アプリはスマホ依存症の克服を支援する **完全ロック型集中モード** を提供する。Lock Task Mode や Device Owner には頼らず、以下の必須権限（オーバーレイ／使用状況アクセス／正確なアラーム）を組み合わせたソフトロックでユーザー操作を封じる。Android 12 以降では `SCHEDULE_EXACT_ALARM` を必須化してロック終了・ウォッチドッグを正確に起動する。UI は Jetpack Compose（黒 × 黄色テーマ）で実装し、Android 11 (API 30) 以降をサポートする。詳細なロードマップと評価対象の技術リストは常に [`IMPLEMENTATION_PLAN.md`](IMPLEMENTATION_PLAN.md) を参照すること。  
...
- `startLock()` で権限チェック（正確アラーム含む）と Android 13 以降の `POST_NOTIFICATIONS` 要求を行い、ロック開始/終了時刻を算出して DataStore・Direct Boot・Device Protected DataStore へ書き込んだ上で Watchdog ハートビートとロック終了アラームを予約する。
